/*! mage version: 0.0.83, 26-08-2018 */
function Class(a, b) {
...
SPE.TypedArrayHelper = function(a, b, c, d) {
    ...
    M.modelsEngine.load(), M.fx.shadersEngine.load(), M.assetsManager.checkInterval = setInterval(M.assetsManager.check, 100);
}, M.assetsManager.loadingMessage = function(a) {}, M.assetsManager.check = function() {
    M.assetsManager.completed.sound && M.assetsManager.completed.video && M.assetsManager.completed.images && M.assetsManager.completed.models ? (M.assetsManager.loadingMessage(!0), 
...
function() {
...
}, M.lightEngine.init(), window.M = window.M || {}, M.modelsEngine = {
    loader: new THREE.JSONLoader(),
    numModels: 0,
    modelsLoaded: 0,
    load: function() {
        M.modelsEngine.map = new HashMap(), M.modelsEngine.models = [];
        for (var a in Assets.Models) M.modelsEngine.numModels++, M.modelsEngine.loadSingleFile(a, Assets.Models[a]);
        0 == M.modelsEngine.numModels && (M.assetsManager.completed.models = !0);
    },
    get: function(a) {
        var b = M.modelsEngine.map.get(a) || !1;
        return !!b && (b.material.wireframe = !1, new Mesh(b.geometry, b.material));
    },
    loadSingleFile: function(a, b) {
        M.modelsEngine.loader.load(b, function(b, c) {
            var d;
            if (c && c.length > 0) {
                c[0].morphTargets = !0, d = new THREE.MultiMaterial(c);
            } else d = new THREE.MeshLambertMaterial({
                wireframe: !0
            });
            var e = {
                geometry: b,
            ...
            M.modelsEngine.map.put(a, e), M.modelsEngine.modelsLoaded++, M.modelsEngine.checkLoad();
        });
    },
    checkLoad: function() {
        M.modelsEngine.modelsLoaded == M.modelsEngine.numModels && (M.assetsManager.completed.models = !0);
    },
    add: function(a) {
        M.modelsEngine.models.push(a);
    }
    ...
    instance: function(a) {
        var b = new SPE.Group({
            ...
            position: {
                value: a.positionValue || new THREE.Vector3(0, 0, -50),
                spread: a.positionSpread || new THREE.Vector3(0, 0, 0)
            },
            acceleration: {
                value: a.accelerationValue || new THREE.Vector3(0, -10, 0),
                spread: a.accelerationSpread || new THREE.Vector3(10, 0, 10)
            },
            velocity: {
                value: a.velocityValue || new THREE.Vector3(0, 25, 0),
                spread: a.velocitySpread || new THREE.Vector3(10, 7.5, 10)
            },
            color: {
                value: a.colorValue || [ new THREE.Color("white"), new THREE.Color("red") ]
            },
            size: {
                value: a.sizeValue || 10
            },
            particleCount: a.particleCount || 2e3
        });
        return b.addEmitter(c), b.clock = new THREE.Clock(), b.render = function() {
            b.tick(b.clock.getDelta());
        }, b;
    }
    ...
    instance: function(a) {
        var b = new SPE.Group({
            ...
            particleCount: a.particleCount || 750,
            maxAge: {
                value: a.maxAge || 3
            },
            position: {
                value: a.positionValue || new THREE.Vector3(0, -15, -50),
                spread: a.positionSpread || new THREE.Vector3(100, 30, 100)
            },
            velocity: {
                value: a.velocityValue || new THREE.Vector3(0, 0, 30)
            },
            wiggle: {
                spread: a.wiggle || 10
            },
            size: {
                value: a.sizeValue || 75,
                spread: a.sizeSpread || 50
            },
            opacity: {
                value: a.opacityValue || [ 0, 1, 0 ]
            },
            color: {
                value: a.colorValue || new THREE.Color(1, 1, 1),
                spread: a.colorSpread || new THREE.Color(.1, .1, .1)
            },
            angle: {
                value: a.angleValue || [ 0, .125 * Math.PI ]
            }
        });
        return b.addEmitter(c), b.clock = new THREE.Clock(), b.render = function() {
            b.tick(b.clock.getDelta());
    ...
    instance: function(a, b, c, d) {
        ...
        e.prototype.constructor = e, e.prototype.render = function() {
            ...
            var g = new e(a, b, c, {
                textureWidth: d.textureWidth || 512,
                textureHeight: d.textureHeight || 512,
                waterNormals: f,
                alpha: 1,
                sunDirection: new THREE.Vector3(-.5773502691896258, .5773502691896258, -.5773502691896258),
                sunColor: 16777215,
                waterColor: 7695,
                distortionScale: d.distortionScale || 50
            }), h = d.width || 512, i = d.height || 512, j = new THREE.Mesh(new THREE.PlaneBufferGeometry(500 * h, 500 * i), g.material);
            return j.add(g), j.rotation.x = .5 * -Math.PI, j.render = function() {
    ...
    instance: function() {
        ...
        return a.prototype.generateMesh = function() {
            ...
            var f = e.geometrySize || 512, g = e.resolution || 1024, h = g / 2, i = -f / 2, j = -f / 2;
            return ocean = new a(b, c, d, {
                USE_HALF_FLOAT: !0,
                INITIAL_SIZE: e.initial.size || 256,
                INITIAL_WIND: e.initial.wind || [ 10, 10 ],
                INITIAL_CHOPPINESS: e.initial.choppiness || 1.5,
                CLEAR_COLOR: e.clearColor || [ 1, 1, 1, 0 ],
                GEOMETRY_ORIGIN: [ i, j ],
                SUN_DIRECTION: e.sunDirection || [ -1, 1, 1 ],
                OCEAN_COLOR: e.oceanColor || new THREE.Vector3(.004, .016, .047),
                SKY_COLOR: e.skyColor || new THREE.Vector3(3.2, 9.6, 12.8),
                EXPOSURE: e.exposure || .35,
