...
const getFountainRate = () =>
    new Proton.Rate(new Proton.Span(5, 10), new Proton.Span(0.05, 0.1));
...
export default class Fountain extends ProtonParticleEmitter {
    constructor(options = {}) {
        ...
        this.setParticleConfig({ texture, size, strength, gravity, colors, autoEmit, emitWhenEditing });
    }

    rebuild() {
        const config = this.getParticleConfig();
        const { texture, size = 5, strength = 25, gravity = 3, colors = [PALETTES.FRENCH.FLAT_FLESH, PALETTES.FRENCH.MELON_MELODY] } = config;
