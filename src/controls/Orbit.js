...
import { Vector3, Vector2, Spherical, Quaternion, MOUSE, EventDispatcher } from "three";
...
export default class Orbit extends EventDispatcher {
    ...
    setMinAzimuthAngle(angle = -Infinity) {
        this.minAzimuthAngle = angle;
    }

    setMaxAzimuthAngle(angle = Infinity) {
        this.maxAzimuthAngle = angle;
    }

    setMinPolarAngle(angle = 0) {
    ...
    setMinDistance(distance = 0) {
        this.minDistance = distance;
    }

    setMaxDistance(distance = Infinity) {
    ...
    onKeyDown = event => {
        if (!this.enabled || this.enableKeys === false || this.enablePan === false) return;
